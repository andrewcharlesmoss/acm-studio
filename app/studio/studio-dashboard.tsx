  ...
  return (
    <div className="studio-shell dashboard-shell">
      <header className="studio-header">
        ...
        <div className="studio-actions"><a className="button-secondary" href="https://andrewmoss.me/">View Andrew Moss ↗</a><a className="button-primary" href="/studio">Open Studio</a></div>
      </header>
      ...
      <main className="dashboard-main">
        ...
        <section className="dashboard-section" aria-labelledby="sites-title">
          <div className="dashboard-section-heading"><div><p className="eyebrow">Site registry</p><h2 id="sites-title">Your sites</h2></div><p>One view of the projects that sit below the ACM identity.</p></div>
          <div className="dashboard-site-grid">
            {sites.map((site) => (
              <article className="dashboard-site-card" key={site.name}>
                ...
                {site.href ? <a className="dashboard-card-link" href={site.href}>Visit site <span aria-hidden="true">↗</span></a> : <span className="dashboard-card-link is-muted">Integration boundary to define</span>}
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="dashboard-section dashboard-tools" aria-labelledby="tools-title">
          ...
          <div className="dashboard-tool-grid">
            <a className="dashboard-tool-card" href="/studio"><span className="dashboard-tool-icon" aria-hidden="true">✎</span><span><strong>Content Studio</strong><small>Create and edit pages and posts with portable blocks.</small></span><span aria-hidden="true">→</span></a>
            <a className="dashboard-tool-card" href="/studio"><span className="dashboard-tool-icon" aria-hidden="true">▧</span><span><strong>Files and media</strong><small>Manage the local media library from the Studio.</small></span><span aria-hidden="true">→</span></a>
            <a className="dashboard-tool-card" href="/studio"><span className="dashboard-tool-icon" aria-hidden="true">⇣</span><span><strong>Backup and restore</strong><small>Protect local content before the persistence layer arrives.</small></span><span aria-hidden="true">→</span></a>
          </div>
        </section>
