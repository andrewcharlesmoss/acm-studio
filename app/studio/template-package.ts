...
import { copyTemplateData, duplicateTemplateSet, templateId, templateMediaIds, visitTemplateNodes, validateTemplateSet, validateTemplateStore, LEGACY_TEMPLATE_VERSION, LEGACY_TEMPLATE_VERSION_2, LEGACY_TEMPLATE_VERSION_3, LEGACY_TEMPLATE_VERSION_4, TEMPLATE_STORAGE_KEY, TEMPLATE_VERSION, type TemplateSet } from "./template-model";
...
export type TemplatePackage = { format: "acm-studio-template-set"; version: typeof TEMPLATE_VERSION | typeof LEGACY_TEMPLATE_VERSION_4 | typeof LEGACY_TEMPLATE_VERSION_3 | typeof LEGACY_TEMPLATE_VERSION_2 | typeof LEGACY_TEMPLATE_VERSION; set: TemplateSet; media: StudioBackupAsset[] };
export const TEMPLATE_PACKAGE_LIMIT = 50 * 1024 * 1024;
export function validateTemplatePackage(value: unknown): TemplatePackage {
  if (!isRecord(value) || value.format !== "acm-studio-template-set" || !([TEMPLATE_VERSION, LEGACY_TEMPLATE_VERSION_4, LEGACY_TEMPLATE_VERSION_3, LEGACY_TEMPLATE_VERSION_2, LEGACY_TEMPLATE_VERSION] as readonly string[]).includes(value.version as string) || !Array.isArray(value.media)) throw new Error("This is not a supported template package.");
