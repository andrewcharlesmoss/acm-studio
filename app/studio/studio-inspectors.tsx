...
export function BlockInspector({ block, onChange, onOpenFiles, canOpenFiles }: { block: ContentBlock; onChange: (block: ContentBlock) => void; onOpenFiles: () => void; canOpenFiles: boolean }) {
  ...
  return (
    <div className="inspector-sections">
      ...
      {block.type === "section" ? <section><h2>Section</h2><label><span>Arrangement</span><select value={block.layout} onChange={(event) => onChange({ ...block, layout: event.target.value as Extract<typeof block.layout, string> })}><option value="stack">Stack</option><option value="row">Row</option><option value="columns">Columns</option></select></label><p className="setting-note">This section contains {block.children.length} nested block{block.children.length === 1 ? "" : "s"}.</p>{block.source ? <><p className="setting-note">Source: {block.source.module} · {block.source.exportName} · {block.source.revision.slice(0, 8)}</p>{canOpenFiles ? <button className="choose-media-button" type="button" onClick={onOpenFiles}>Open source files</button> : null}</> : null}</section> : null}
      {block.type === "group" ? <section><h2>Layout</h2><label><span>Arrangement</span><select value={block.layout} onChange={(event) => onChange({ ...block, layout: event.target.value as Extract<typeof block.layout, string> })}><option value="stack">Stack</option><option value="row">Row</option><option value="columns">Columns</option></select></label><p className="setting-note">This group contains {block.children.length} nested block{block.children.length === 1 ? "" : "s"}.</p></section> : null}
