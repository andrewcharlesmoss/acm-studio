  ...
  return (
    <div className="inspector-sections">
      {isDocumentPanel ? <>
        ...
        {document.kind === "post" ? <InspectorSection kind={document.kind} title="Excerpt"><label><span>Excerpt (optional)</span><textarea rows={4} value={document.excerpt} onChange={(event) => onChange("excerpt", event.target.value)} placeholder="A short public summary; otherwise, we’ll use the opening content." /></label></InspectorSection> : null}
        {document.kind === "page" ? <section><h2>Address</h2><label><span>Slug</span><input value={document.slug} onChange={(event) => onChange("slug", event.target.value.toLowerCase().replace(/[^a-z0-9-]+/g, "-"))} /></label></section> : null}
        <InspectorSection kind={document.kind} title="Author"><label><span>Author</span><select value={resolvedDocument.author ?? ""} onChange={(event) => { onFieldOverride?.("author", false); onChange("author", event.target.value); }}><option value="">No author</option>{resolvedDocument.author && resolvedDocument.author !== "Andrew Moss" ? <option value={resolvedDocument.author}>{resolvedDocument.author}</option> : null}<option value="Andrew Moss">Andrew Moss</option></select></label></InspectorSection>
        {documentControls}
