...
export function StudioPrototype() {
  ...
  return (
    <div className="studio-shell studio-desktop-only" onBeforeInputCapture={(event) => { if (!writable && (event.target as HTMLElement).isContentEditable) event.preventDefault(); }} onPasteCapture={(event) => { if (!writable && (event.target as HTMLElement).isContentEditable) event.preventDefault(); }} onCutCapture={(event) => { if (!writable && (event.target as HTMLElement).isContentEditable) event.preventDefault(); }}>
      ...
      <main className={`studio-workspace${previewing ? " is-previewing" : ""}${studioSection === "files" || studioSection === "backup" || studioSection === "bin" ? " is-tool" : ""}${studioSection === "templates" ? " template-workspace" : ""}`} style={{ "--studio-library-width": `${studioSection === "templates" ? 290 : libraryPaneCollapsed ? 0 : libraryPaneWidth}px`, "--studio-inspector-width": `${studioSection === "templates" ? 300 : inspectorPaneCollapsed ? 0 : inspectorPaneWidth}px` } as CSSProperties}>
