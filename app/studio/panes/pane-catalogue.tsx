  ...
  return <main className="pl-catalogue">
    ...
    <div className="pl-catalogue-layout">
      ...
      <section className="pl-main" aria-label="Pane Specimens">
        ...
        <details className="pl-source"><summary>Source reference · {snapshots.commit.slice(0, 7)}</summary><p>Captured {snapshots.capturedAt}. Maintained examples, not live product connections.</p>{studio && <p>{definition.sourceComponent} · {definition.width}px<br /><code>{definition.source}</code></p>}<p>All examples use shared skeleton components. Fixed headers, consistent spacing and always-available edge controls are intentional standardisations.</p><ul>{snapshots.files.map((file) => <li key={file.path}><code>{file.path}</code><br /><span className="pl-hash">SHA-256 {file.sha256}</span></li>)}</ul></details>
      </section>
